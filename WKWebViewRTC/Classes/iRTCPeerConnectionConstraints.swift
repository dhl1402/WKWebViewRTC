...
class iRTCPeerConnectionConstraints {
	...
	init(pcConstraints: NSDictionary?) {
		...
		if _mandatoryConstraints != nil {
			mandatoryConstraints = iRTCPeerConnectionConstraints.parseConstraints(constraints: _mandatoryConstraints!)
		}
		...
		if _optionalConstraints != nil {
			optionalConstraints = iRTCPeerConnectionConstraints.parseOptionalConstraints(optionalConstraints: _optionalConstraints!)
		}
		
		// Handle constraints at the root of pcConstraints
		if pcConstraints != nil && _optionalConstraints == nil && _mandatoryConstraints == nil  {
			mandatoryConstraints = iRTCPeerConnectionConstraints.parseConstraints(constraints: pcConstraints!)
		}
		
		NSLog("iRTCPeerConnectionConstraints#init() | [mandatoryConstraints:%@, optionalConstraints:%@]",
	...
	deinit {
		NSLog("iRTCPeerConnectionConstraints#deinit()")
	}

	func getConstraints() -> RTCMediaConstraints {
		NSLog("iRTCPeerConnectionConstraints#getConstraints()")
