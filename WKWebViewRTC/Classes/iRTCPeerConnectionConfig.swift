...
class iRTCPeerConnectionConfig {
	...
	init(pcConfig: NSDictionary?) {
		NSLog("iRTCPeerConnectionConfig#init()")
		...
		if iceServers != nil {
			for iceServer: NSDictionary in iceServers! {
				...
				if (urls != nil && urls!.count > 0) {
					NSLog("iRTCPeerConnectionConfig#init() | adding ICE server [url:'%@', username:'%@', password:'******']",
	...
	deinit {
		NSLog("iRTCPeerConnectionConfig#deinit()")
	}
	
	func getConfiguration() -> RTCConfiguration {
		NSLog("iRTCPeerConnectionConfig#getConfiguration()")
